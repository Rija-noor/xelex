document.addEventListener("DOMContentLoaded", () => {
  const categories = [
    { title: "Male Hoodies", file: "malehoodies.json", containerId: "male-hoodies" },
    { title: "Female Special", file: "femalespecial.json", containerId: "female-special" },
    { title: "Customized Tees & Hoodies", file: "customizedTees.json", containerId: "customized" },
    { title: "Couple Fits", file: "coupleFits.json", containerId: "couple-fits" }
  ];

  categories.forEach(category => {
    fetch(`assets/js/${category.file}`)
  ...
  <span class="text-white font-semibold">${item.newPrice}</span>
      ...
      .catch(error => console.error(`Failed to load ${category.title}`, error));
  });

  // ✅ Popup logic must be INSIDE DOMContentLoaded
  ...
  orderForm.addEventListener("submit", (e) => {
    ...
    const phoneNumber = "923162639541"; // 🔹 Replace with your real WhatsApp number
